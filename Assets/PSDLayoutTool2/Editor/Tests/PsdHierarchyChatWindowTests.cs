namespace PsdLayoutTool2.Tests
{
    ...
    public sealed class PsdHierarchyChatWindowTests
    {
        ...
        public void CopyMessageToClipboardPreservesTheCompleteErrorMessage()
        {
            ...
            // 无头/被占用的会话可能拿不到系统剪贴板（写入静默失败）。此时本用例无法验证内容，
            // 明确跳过并给出原因，而不是把环境问题伪装成代码回归。
            if (EditorGUIUtility.systemCopyBuffer != errorMessage &&
                string.IsNullOrEmpty(EditorGUIUtility.systemCopyBuffer))
            {
                Assert.Ignore(
                    "当前会话无法访问系统剪贴板（通常由其它 Unity 实例占用）。剪贴板可用时本用例验证完整错误内容。");
            }
