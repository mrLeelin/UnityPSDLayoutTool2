...
public static class fsPortableReflection
{
	...
	public static Attribute GetAttribute(MemberInfo element, Type attributeType, bool shouldCache)
	{
		...
		if (!_cachedAttributeQueries.TryGetValue(key, out var value))
		{
			...
			{
				value = (Attribute)customAttributes.First();
			}
			if (shouldCache)
			{
	...
	public static PropertyInfo GetDeclaredProperty(this Type type, string propertyName)
	{
		...
		while (true)
		{
			if (num < declaredProperties.Length)
			{
				if (declaredProperties[num].Name == propertyName)
				{
					break;
				}
				num++;
	...
	public static MemberInfo GetDeclaredMember(this Type type, string memberName)
	{
		...
		while (true)
		{
			if (num < declaredMembers.Length)
			{
				if (declaredMembers[num].Name == memberName)
				{
					break;
				}
				num++;
	...
	static fsPortableReflection()
	{
		...
		DeclaredFlags = BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static;
	}
}
}
