...
public class fsSerializer
{
	internal class fsLazyCycleDefinitionWriter
	{
		...
		public fsLazyCycleDefinitionWriter()
		{
			...
			_pendingDefinitions = new Dictionary<int, fsData>();
			_references = new HashSet<int>();
		}
	}
	...
	private fsBaseConverter GetConverter(Type type, Type overrideConverterType)
	{
		...
		{
			if (!_cachedConverters.TryGetValue(type, out var value))
			{
				fsObjectAttribute attribute = fsPortableReflection.GetAttribute<fsObjectAttribute>(type);
				...
				value = new fsForwardConverter(attribute2);
				value.Serializer = this;
				return _cachedConverters[type] = value;
			}
			return value;
		}
		if (!_cachedConverterTypeInstances.TryGetValue(overrideConverterType, out var value2))
		{
			value2 = (fsBaseConverter)Activator.CreateInstance(overrideConverterType);
			value2.Serializer = this;
			_cachedConverterTypeInstances[overrideConverterType] = value2;
		}
		return value2;
	}
